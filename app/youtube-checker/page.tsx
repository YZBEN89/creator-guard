...
function analyzeContent(text: string): AnalysisResult {
   ...
   *
   * This is important because optimization-only patterns such as
   * "make you rich overnight" must never hide a real risk.
   */
  const keywordMatches = riskCategories.flatMap((category) =>
    findMatches(text, category, "keyword")
  );

  const scoringPatternMatches = patternCategories
    .filter((category) => !category.optimizationOnly)
    .flatMap((category) =>
  ...
  for (const match of realRiskMatches) {
    if (hasReducingContext(text, match.start)) {
   ...
   *
   * If an optimization phrase overlaps with a real risk phrase,
   * the real risk takes priority.
   */
  const optimizationRawMatches = patternCategories
    ...
    .filter((match) => !hasReducingContext(text, match.start));

  const optimizationMatches = removeOverlappingMatches(
    optimizationRawMatches.filter(
      (optimizationMatch) =>
        !realRiskMatches.some(
          (riskMatch) =>
            optimizationMatch.start < riskMatch.end &&
            optimizationMatch.end > riskMatch.start
        )
    )
  );
   ...
   */
  const highMatches = activeRiskMatches.filter(
    (match) => match.level === "High"
  );

  const mediumMatches = activeRiskMatches.filter(
    ...
    <main className="min-h-screen bg-white text-zinc-900">
      ...
      <section className="mx-auto max-w-4xl px-6 pb-10 pt-16 text-center">
        ...
        <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-zinc-600">
          Check video titles, descriptions, scripts, and promotional
          content for potentially sensitive language and
          monetization-related risks.
        </p>
      </section>
          ...
          <div className="mt-8 space-y-6">
              ...
              <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-6">
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    These detected patterns contribute to the
                    current risk score.
                  </p>
                </div>
              ...
              <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
                <div className="mb-6">
                  ...
                  <p className="mt-2 text-sm leading-6 text-zinc-500">
                    These expressions are not treated as direct
                    safety risks, but improving them may make your
                    content clearer and more credible.
                  </p>
                </div>
            ...
            {result.reducedMatches.length > 0 && (
              <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-500">
                  Some phrases appeared in a context that may
                  reduce their apparent risk, such as warnings,
                  criticism, or educational discussion. This is a
                  basic context signal and should not be treated as
            ...
            <section className="rounded-3xl border border-zinc-200 bg-zinc-50 p-6 sm:p-8">
              ...
              <ul className="mt-4 space-y-3 text-sm leading-6 text-zinc-600">
                <li>
                  • Avoid absolute promises or claims that guarantee
                  a specific outcome.
                </li>
                ...
                <li>
                  • Review the full context of your content rather
            ...
            <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6 sm:p-8">
              ...
              <p className="mt-3 text-sm leading-6 text-amber-800">
                This assessment is based on predefined content
                patterns and basic context signals. It is intended as
                general guidance and does not represent or guarantee
                the moderation, monetization, or advertising
                decisions of YouTube or any other platform.
              </p>
            </section>
          </div>
        )}
